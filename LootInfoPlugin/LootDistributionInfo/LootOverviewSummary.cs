...
public sealed class LootOverviewSummary
{
    ...
    public int TotalRollEntries { get; init; }

    public int EntriesWithRolls { get; init; }

    public DateTimeOffset? LatestItemAtUtc { get; init; }

    public IReadOnlyList<LootOverviewBucket> TopZones { get; init; } = [];

    public IReadOnlyList<LootOverviewBucket> TopRecipients { get; init; } = [];
    ...
    public static LootOverviewSummary Build(IEnumerable<LootRecord> records)
    {
        ...
        return new LootOverviewSummary
        {
            ...
            UniqueItems = recordList
                ...
                .Count(),
            TotalRollEntries = recordList.Sum(record => record.RollEntries.Count),
            EntriesWithRolls = recordList.Count(record => record.RollEntries.Count > 0),
            ...
            TopRecipients = BuildBuckets(recordList, record => record.WhoDisplayName ?? record.WhoName ?? "Unknown"),
    ...
    private static IReadOnlyList<LootOverviewBucket> BuildBuckets(IEnumerable<LootRecord> records, Func<LootRecord, string> selector, int limit = 5)
    {
        return records
            ...
            .Select(group => new LootOverviewBucket(
                group.First(),
                group.Key,
                group.Count(),
                group.Sum(record => record.RollEntries.Count)))
            .OrderByDescending(bucket => bucket.Count)
            .ThenByDescending(bucket => bucket.RollCount)
...
public sealed record LootOverviewBucket(LootRecord SampleRecord, string Label, int Count, int RollCount);
